...
export const CHILLED_WATER_TABLE = [
...
export const COOLING_WATER_TABLE = [
...
export function lookupPipeSize(sysType = "chilled", rtValue = null, lpmValue = null, customFactor = null) {
  const isChilled = sysType === "chilled";
  const defaultFactor = isChilled ? 10.08 : 12.8;
  const factor = customFactor !== null && customFactor > 0 ? customFactor : defaultFactor;
  const table = isChilled ? CHILLED_WATER_TABLE : COOLING_WATER_TABLE;
  ...
  if (rtValue !== null && rtValue !== "") {
    ...
    lpm = rt * factor;
  } else if (lpmValue !== null && lpmValue !== "") {
    lpm = parseFloat(lpmValue) || 0;
    rt = factor > 0 ? lpm / factor : 0;
  }

  // Find matching pipe size in table
